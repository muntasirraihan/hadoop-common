...
public class MRClientProtocolPBServiceImpl implements BlockingInterface {
  ...
  @Override
  public ResumeTaskResponseProto resumeTask(RpcController controller,
      ResumeTaskRequestProto proto) throws ServiceException {
    ResumeTaskRequest request = new ResumeTaskRequestPBImpl(proto);
    try {
      ResumeTaskResponse response = real.resumeTask(request);
      return ((ResumeTaskResponsePBImpl)response).getProto();
    } catch (YarnRemoteException e) {
      throw new ServiceException(e);
    }
  }  
